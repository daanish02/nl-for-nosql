    ...
    def __init__(self):
        ...
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a helpful AI chatbot. You are provided with tools to answer questions about movies. Think step-by-step and use these tools to get the information required to answer the user query. \nDo not re-run tools unless absolutely necessary. If you are not able to get enough information using the tools, reply with I DON'T KNOW. \nYou have access to the following tools: {tool_names}.",
                ),
    ...
    def _tools_node(self, state: GraphState) -> Dict[str, List]:
        ...
        for tool_call in tool_calls:
            try:
                ...
                result.append(
                    ToolMessage(content=f"Tool error: {str(e)}", tool_call_id=tool_id)
                )
